...
bool Simplex::MyEntity::GetIsTile()
{
	return isTile;
}
void Simplex::MyEntity::SetIsTile(bool tile)
{
	isTile = tile;
}
//The big 3
...
//	}
//}
Simplex::MyEntity::MyEntity(String a_sFileName, bool a_bGravity, bool isStageTile, bool dynamic, float a_fMass, float speedMultiplier, String a_sUniqueID)
{
	Init();
	entName = a_sFileName;
	m_pModel = new Model();
	...
	if (m_pModel->GetName() != "")
	{
		...
		m_sUniqueID = a_sUniqueID;
		m_IDMap[a_sUniqueID] = this;
		m_pRigidBody = new MyRigidBody(m_pModel->GetVertexList(), dynamic); //generate a rigid body
		m_pRigidBody->SetGravity(a_bGravity);
		m_pRigidBody->SetMass(a_fMass);
		m_pRigidBody->SetSpeedMultiplier(speedMultiplier);
		m_bInMemory = true; //mark this entity as viable
	}
	isTile = isStageTile;
}
Simplex::MyEntity::MyEntity(MyEntity const& other)
{
	...
	m_pRigidBody = new MyRigidBody(m_pModel->GetVertexList(), false); 
...
void Simplex::MyEntity::MoveEntity(void) 
{
	SetModelMatrix(m_pRigidBody->GetModelMatrix());
}

bool Simplex::MyEntity::GetIsPlayer(void){ return m_bIsPlayer; }
void Simplex::MyEntity::SetIsPlayer(bool a_bIsPlayer) { m_bIsPlayer = a_bIsPlayer; }