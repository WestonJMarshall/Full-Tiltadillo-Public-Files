...
class MyEntityManager
{
	...
	int goalIndex = 0; //gets location of goal structure
	bool victory = false;
	bool died = false;
	int level = 0;
	std::vector<String> Levels = { "Level1.txt", "Level2.txt" };
public:
	...
	*/
	...
	-	String a_sFileName -> Name of the model to load
	-	String a_sUniqueID -> Name wanted as identifier, if not available will generate one
	-	bool a_bGravity -> Whether the Entity is affected by gravity
	-	int a_iMass -> the mass of the Entity
	OUTPUT: ---
	*/
	void AddEntity(String a_sFileName, bool a_bGravity = false, bool isStageTile = false, bool dynamic = false, float a_iMass = 0, float speedMultiplier = 0, String a_sUniqueID = "NA");
	/*
	...
	*/
	...
	ARGUMENTS: uint maxOctreeSubdivision - how far the octree should subdivide on its own
	OUTPUT: ---
	*/
	void Update(int maxOctreeSubdivision, vector3 playerPosition, vector2 rotation);
	/*
	...
	*/
	...
	/*
	USAGE: Will add the octtree to the render list
	ARGUMENTS:
	-	bool a_bRigidBody = false -> draw outline?
	OUTPUT: ---
	*/
	void AddOctreeToRenderList(bool a_bOutline = false);
	/*
	...
	*/
	...
	/*
	USAGE: recreate octree and all of its node with current array of entities
	ARGUMENTS: 
	-   int m_uOctantLevels -> how many subdivisions to make the octree
	OUTPUT: ---
	*/
	void RecreateOctree(int m_uOctantLevels);
	void SetGoalIndex(int counter);
	int GetGoalIndex();
	bool GetVictoryStatus();
	bool GetDeathStatus();
	void Reset();
	/*
	USAGE: Toggles the BoundingSphere of all entities
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void ToggleBoundingSphere(void);
	/*
	USAGE: Toggles the oriented bounding box of all entities
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void ToggleOrientedBoundingBox(void);
	/*
	USAGE: Toggles the axis realigned bounding box of all entities
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void ToggleAxisRealignedboundingBox(void);
private:
	/*
	Usage: constructor
	Arguments: ---
