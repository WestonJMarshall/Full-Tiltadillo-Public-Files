...
void Application::ProcessKeyReleased(sf::Event a_event)
{
	...
	case sf::Keyboard::PageUp:
		++(m_pEntityMngr->m_uOctantID);
		if (m_pEntityMngr->m_uOctantID >= m_pEntityMngr->octree->OctreeSize()) 
		{
			//loop around
			m_pEntityMngr->m_uOctantID = -1;
		}

		break;
	case sf::Keyboard::PageDown:
		--(m_pEntityMngr->m_uOctantID);
		if (m_pEntityMngr->m_uOctantID < -1) 
		{
			//loop around
			m_pEntityMngr->m_uOctantID = m_pEntityMngr->octree->OctreeSize() - 1;
		}

		break; 
	case sf::Keyboard::O:
		m_uOctantOutline = !m_uOctantOutline;
		break;
	case sf::Keyboard::Add:
		if (m_uOctantLevels < 4)
		{
			...
			++m_uOctantLevels;
			m_pEntityMngr->octree->CreateSubdivisions(m_uOctantLevels);

			//outline all octants when changing subdivisions
			m_pEntityMngr->m_uOctantID = -1;
		}
		break;
