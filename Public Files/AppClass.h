...
}//source: https://stackoverflow.com/a/1149623
...
class Application
{
	...
	bool m_uOctantOutline = false; //Number of levels in the octree
...
private:
	...
	vector3 pastValidEntityVelocity = vector3(0, 0, 1);

	//for keeping track of levels
	int level = 0;
	std::vector<String> Levels = {"Level0.txt", "Level1.txt", "Level2.txt", "Level3.txt", "Level4.txt", "Level5.txt"};

	//used for stage rotation storing so it's smoother
	float valFB = 0;
	float valLR = 0; 
	vector2 tiltValues;
	sf::Clock clock;
...
#pragma region Initialization / Release
	...
	*/
	...
	/*
	USAGE: calculates the tilting values based on player input
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	...
	*/
	void Release(void);
	void Reset(void); //calls Release and Init to reset entity manager
#pragma endregion
...
#pragma region Application Controls
	...
	*/
	...
	/*
	USAGE: Moves camera to behind the player entity
	ARGUMENTS: MyEntity* PlayerEntity = Player entity to get behind
	OUTPUT: ---
	*/
	void CameraFollow(MyEntity* PlayerEntity);
	/*
	USAGE: Moves camera high above the stage looking down while following the player. Does not rotate
	ARGUMENTS: MyEntity* PlayerEntity = Player entity to follow
	OUTPUT: ---
	*/
	void CameraBirdsEye(MyEntity* PlayerEntity);
#pragma endregion
...
#pragma region GUI
	...
	*/
	void DrawGUI(void);
	/*
	USAGE: //draws debug GUI
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void DrawLevelGUI(void);
	/*
	USAGE: //draws GUI thats always on scene
	ARGUMENTS: ---
	OUTPUT: ---
	*/
