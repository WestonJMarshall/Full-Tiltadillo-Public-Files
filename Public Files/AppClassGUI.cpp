...
#pragma endregion
	...
	if (m_bGUI_Main)
	{
		...
		{
			ImGui::Text("Programmers: Team WeTried\n");
			...
			ImGui::Text("	 O: Toggles Octant"); 
			ImGui::Separator();
			ImGui::Text("    PageUp: Increment Octant display\n");
			ImGui::Text("    PageDw: Decrement Octant display\n");

			ImGui::Separator();
			ImGui::TextColored(ImColor(255, 255, 0), "Debug\n");
			ImGui::Text("	 F1: Next Level\n");
			ImGui::Text("	 F2: Previous Level\n");
			ImGui::Separator();
			ImGui::Text("	 B: Toggles Bounding Spheres\n");
			ImGui::Text("	 N: level Oriented Bounding Boxes\n");
			ImGui::Text("	 M: level Axis Realigned bounding Boxes\n");
			ImGui::Separator();

			ImGui::TextColored(ImColor(255, 255, 0), "Player Info\n");
			ImGui::TextColored(ImColor(255, 0, 0), "Arrow Keys: Movement\n");
			ImGui::Separator();
			ImGui::Text("    Time Remaining: %.2f", timer); //displays countdown timer 
			ImGui::Separator();
			ImGui::Text("    Player Velocity X %.2f\n", playerEntity->GetRigidBody()->GetVelocity().x); 
			ImGui::Text("    Player Velocity Y %.2f\n", playerEntity->GetRigidBody()->GetVelocity().y); 
			ImGui::Text("    Player Velocity Z %.2f\n", playerEntity->GetRigidBody()->GetVelocity().z); 
			ImGui::Separator();
			ImGui::Text("    Player Normal Force X %.2f\n", playerEntity->GetRigidBody()->GetDynamicVelocity().x); 
			ImGui::Text("    Player Normal Force Y %.2f\n", playerEntity->GetRigidBody()->GetDynamicVelocity().y); 
			ImGui::Text("    Player Normal Force Z %.2f\n", playerEntity->GetRigidBody()->GetDynamicVelocity().z); 
		}
		ImGui::End();
	}
...
void Simplex::Application::DrawLevelGUI(void)
{
	NewFrame();
	m_pMeshMngr->Print("						Level: ", C_BLACK);//Add a line on top
	m_pMeshMngr->PrintLine(std::to_string(level), C_RED);
	m_pMeshMngr->Print("					Time Remaining : ", C_BLACK);//Add a line on top
	std::string timerString(16, '\0');
	auto written = std::snprintf(&timerString[0], timerString.size(), "%.2f", timer);
	timerString.resize(written);
	m_pMeshMngr->PrintLine(timerString);

	uint nEmptyLines = 9;
	for (uint i = 0; i < nEmptyLines; ++i)
		m_pMeshMngr->PrintLine("");
	if(victoryUI)
		m_pMeshMngr->Print("						YOU WON! ", C_YELLOW);//Add a line on top
	else
		m_pMeshMngr->PrintLine("");
	nEmptyLines = 10;
	for (uint i = 0; i < nEmptyLines; ++i)
		m_pMeshMngr->PrintLine("");
	m_pMeshMngr->Print("											Q -toggles debug", C_WHITE);
}
