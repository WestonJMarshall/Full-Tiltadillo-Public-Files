...
class MyEntity
{
	...
	matrix4 tileRotation = IDENTITY_M4;
...
public:
	...
	/*
	Usage: Constructor that specifies the name attached to the MyEntity, whether the MyEntity is affected by gravity, and the mass of the MyEntity
	Arguments:
	-	String a_sFileName -> Name of the model to load
	-	String a_sUniqueID -> Name wanted as identifier, if not available will generate one
	-	bool a_Gravity -> Whether the MyEntity is affected by gravity
	-	int a_iMass -> The mass of the MyEntity
	Output: class object instance
	*/
	MyEntity(String a_sFileName, bool a_Gravity = false, bool isStageTile = false, bool dynamic = false, float a_fMass = 0, float reboundPower = 0, String a_sUniqueID = "NA");
	/*
	...
	*/
	...
	/*
	USAGE: Moves the entity
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void MoveEntity(void);
	/*
	USAGE: gets whether the object is the player
	ARGUMENTS: ---
	OUTPUT: whether the object is the player
	*/
	bool GetIsPlayer(void);
	/*
	USAGE: sets whether the object is the player
	ARGUMENTS: whether the object is the player
	OUTPUT: ---
	*/
	void SetIsPlayer(bool a_bIsPlayer);
private:
	/*
